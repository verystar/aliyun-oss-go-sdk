...
func (s *OssBucketSuite) TestWithEmptyObjectKey(c *C) {
	...
	err = s.bucket.DeleteObjectTagging("")
	c.Assert(err.Error(), Equals, "object name is empty")

	_, err = s.bucket.ListObjects()
	c.Assert(err, IsNil)

	_, err = s.bucket.ListObjectVersions()
	c.Assert(err, IsNil)

	_, err = s.bucket.ListObjectsV2()
	c.Assert(err, IsNil)
}
...
func (s *OssBucketSuite) TestPutMultipartObjectLimitSpeed(c *C) {
	...
	textBuffer := RandStr(1024 * 1024)
	if detectSpeed < perTokenBandwidthSize {
		os.WriteFile(fileName, []byte(textBuffer), 0644)
...
func (s *OssBucketSuite) TestGetSingleObjectLimitSpeed(c *C) {
	...
	// Get object and compare content
	fileBody, err := os.ReadFile(tempFile)
	c.Assert(err, IsNil)
